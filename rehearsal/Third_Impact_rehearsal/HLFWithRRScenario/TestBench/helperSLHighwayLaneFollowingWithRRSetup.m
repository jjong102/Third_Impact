...
modelName = "HighwayLaneFollowingRRTestBench";
...
if ~isempty(scenarioSimulationObj)
    ...
    behavior = 'LANE FOLLOWING.rrbehavior';
    ...
    % behaviorCandidates = ["LANE FOLLOWING", "PATH FOLLOWING", "CUSTOMDRIVE", "CUSTOM DRIVE", "DRIVER", "DRIVER MODEL"];
    % 
    % egoBehavior = "";
    % 
    % for i = 1:length(world.behaviors)
    %     refUpper = upper(string(world.behaviors(i).asset_reference));
    %     refUpper = erase(refUpper, ".RRBEHAVIOR"); % 확장자 차이 무시
    %     if any(contains(refUpper, behaviorCandidates))
    %         egoBehavior = world.behaviors(i).id;
    %         break;
    %     end
    % end
    ...
    %     for i = 1:length(world.actors)
    %         id = str2double(world.actors(i).actor_spec.id);
    %         if world.actors(i).actor_spec.behavior_id == egoBehavior
    %             egoId = id;
    %             break;
    %         end
    %     end
    % else
    % 
    %     warning("Ego behavior not found. Falling back to first actor as ego.");
    %     egoId = str2double(world.actors(1).actor_spec.id);
    % end
    % --- find ego behavior by asset reference (case-insensitive) ---
    behavior = "HLF.rrbehavior";                 % 파일명만 넣어도 OK
    egoBehavior = "";
    
    for i = 1:length(world.behaviors)
        ref = string(world.behaviors(i).asset_reference);  % e.g., "Assets/Behaviors/HLF.rrbehavior"
        if contains(upper(ref), upper(behavior))
            egoBehavior = world.behaviors(i).id;
            break;
        end
    end
    
    % --- pick ego actor using the found behavior; fallback if not found ---
    if egoBehavior ~= ""
        for i = 1:length(world.actors)
            id = str2double(world.actors(i).actor_spec.id);
            if world.actors(i).actor_spec.behavior_id == egoBehavior
        ...
        warning("Behavior '%s' not found. Falling back to first actor as ego.", behavior);
        egoId = str2double(world.actors(1).actor_spec.id);
    end
    numActors = length(world.actors);
